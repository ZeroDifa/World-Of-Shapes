...
class Entity {
	...
	initStatsByClass() {
		...
		switch (this.class) {
			case 1:
				this.AllowSpells = ['global', 1, 2, 3];
				this.initCooldowns();
				// basic
				this.stamina = 45;
				this.intelligence = 30;
				this.manaRegenAtRest = 5;
				this.manaRegenAtCombat = 1;
				this.protection = 0.9;
				// 
				this.initStatsByMainCharacteristics()
				this.speed = 4; // per second
				this.radius = 35;
				break
			case 2:
				this.AllowSpells = ['global', 4, 5, 6, 7];
				this.initCooldowns();
				// basic
				this.stamina = 90;
				this.energyRegen = 16;
	...
	getQuarter(angle) {
		...
		else console.log('error angle', angle);
    }
    getSign(a) {
        return a >= 0 ? 1 : -1
    }
